...
try:
    ...
    subprocess.run("pip3 install nltk", shell=False, check=False)
...
try:
    ...
    subprocess.run("python3 -m nltk.downloader stopwords names punkt averaged_perceptron_tagger", shell=False, check=False)
    if subprocess.CompletedProcess.returncode == 0:
...
try:
    ...
    subprocess.run("pip3 install matplotlib", shell=False, check=False)
    if subprocess.CompletedProcess.returncode == 0:
...
try:
    ...
    subprocess.run("pip3 install jsonpickle", shell=False, check=False)
    if subprocess.CompletedProcess.returncode == 0:
